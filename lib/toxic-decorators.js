...
var getOwnKeys = toxicPredicateFunctions.isFunction(getOwnPropertySymbols) ? function (obj) {
...
var getOwnPropertyDescriptors = toxicPredicateFunctions.isFunction(_Object$getOwnPropertyDescriptors) ? _Object$getOwnPropertyDescriptors : function (obj) {
...
function compressMultipleDecorators() {
  ...
  if (fns.length === 1) {
    if (!toxicPredicateFunctions.isFunction(fns[0])) throw new TypeError(fnOnlyErrorMsg);
    return fns[0];
  }
  return function (obj, prop, descirptor) {
    // $FlowFixMe: the reduce will return a descriptor
    return fns.reduce(function (descirptor, fn) {
      if (!toxicPredicateFunctions.isFunction(fn)) throw new TypeError(fnOnlyErrorMsg);
...
function accessor() {
  ...
  return function (obj, prop, descriptor) {
    ...
    var handleGet = function handleGet(value) {
      ...
      return hasGet ? toxicPredicateFunctions.bind(get, this)(value) : value;
    };
    var handleSet = function handleSet(value) {
      // $FlowFixMe: it's really function here
      return hasSet ? toxicPredicateFunctions.bind(set, this)(value) : value;
    };
    if (isAccessorDescriptor(descriptor)) {
      ...
      var getter = hasOriginGet || hasGet ? function () {
        ...
        var boundGetter = toxicPredicateFunctions.bind(handleGet, this);
        var originBoundGetter = function originBoundGetter() {
          return hasOriginGet
          // $FlowFixMe: we have do a check here
          ? toxicPredicateFunctions.bind(originGet, _this)() : undefined;
        };
      ...
      var setter = hasOriginSet || hasSet ? function (val) {
        ...
        var boundSetter = toxicPredicateFunctions.bind(handleSet, this);
        var originBoundSetter = function originBoundSetter(value) {
          return hasOriginSet
          // $FlowFixMe: flow act like a retarded child on optional property
          ? toxicPredicateFunctions.bind(originSet, _this2)(value) : value;
        };
      ...
      return {
        get: function get() {
          var boundFn = toxicPredicateFunctions.bind(handleGet, this);
          if (inited) return boundFn(value);
          value = toxicPredicateFunctions.bind(initializer, this)();
          inited = true;
          return boundFn(value);
        },
        set: function set(val) {
          var boundFn = toxicPredicateFunctions.bind(handleSet, this);
      ...
      return {
        get: function get() {
          return toxicPredicateFunctions.bind(handleGet, this)(_value);
        },
        set: function set(val) {
          var boundFn = toxicPredicateFunctions.bind(handleSet, this);
...
function before() {
  ...
  return function (obj, prop, descriptor) {
    ...
    if (!toxicPredicateFunctions.isFunction(fn)) throw new TypeError('@before can only be used on function');
    var handler = function handler() {
      ...
      var paras = fns.reduce(function (paras, fn) {
        var result = toxicPredicateFunctions.bind(fn, _this).apply(undefined, _toConsumableArray(paras));
        return result === undefined ? paras : toxicPredicateFunctions.isArray(result) ? result
        // $FlowFixMe: what the hell, it can be anything
        : [result];
      }, args);
      return toxicPredicateFunctions.bind(fn, this).apply(undefined, _toConsumableArray(paras));
    };
    return {
      value: handler,
      configurable: configurable,
...
function after() {
  ...
  return function (obj, prop, descriptor) {
    ...
    if (!toxicPredicateFunctions.isFunction(value)) throw new TypeError('@after can only be used on function');
    var handler = function handler() {
      var ret = toxicPredicateFunctions.bind(value, this).apply(undefined, arguments);
      return toxicPredicateFunctions.bind(fn, this)(ret);
    };
    return {
      value: handler,
      configurable: configurable,
...
function initialize() {
  ...
  return function (obj, prop, descriptor) {
    if (descriptor === undefined) {
      return {
        value: toxicPredicateFunctions.bind(fn, obj)(),
        configurable: true,
        writable: true,
        enumerable: true
      };
    ...
    if (isAccessorDescriptor(descriptor)) {
      ...
      return accessor({
        get: function get(value) {
          ...
          return toxicPredicateFunctions.bind(fn, this)(value);
        },
    ...
    if (isInitializerDescriptor(descriptor)) {
      ...
      var handler = function handler() {
        return toxicPredicateFunctions.bind(fn, this)(toxicPredicateFunctions.bind(initializer, this)());
      };
      return {
        initializer: handler,
        configurable: descriptor.configurable,
    ...
    // $FlowFixMe: useless disjoint union
    var value = toxicPredicateFunctions.bind(fn, this)(descriptor.value);
    return {
      value: value,
      // $FlowFixMe: useless disjoint union
      writable: descriptor.writable,
...
function autobindClass() {
  ...
  return function (Klass) {
    if (!toxicPredicateFunctions.isFunction(Klass)) throw new TypeError('@autobindClass can only be used on class');
    var prototype = Klass.prototype;

    if (toxicPredicateFunctions.isVoid(prototype)) throw new Error('The prototype of the class is empty, please check it');
    ...
    for (var i = 0, len = keys.length; i < len; i++) {
      ...
      var desc = descs[key];
      if (key === 'constructor' || !toxicPredicateFunctions.isFunction(desc.value) ||
      // $FlowFixMe: not support symbol yet
      exclude.indexOf(key) > -1) continue;
      // $FlowFixMe: not support symbol yet
      defineProperty$1(prototype, key, autobind(prototype, key, desc));
    }
  };
}
...
function getBoundSuper(obj, fn) {
  ...
  if (superStore.has(fn) === false) {
    // $FlowFixMe: already insure superStore is not undefined
    superStore.set(fn, toxicPredicateFunctions.bind(fn, obj));
  }
  // $FlowFixMe: already insure superStore is not undefined
...
function frozen(obj, prop, descriptor) {
  ...
  if (isAccessorDescriptor(descriptor)) {
    ...
    return {
      get: function get() {
        var value = toxicPredicateFunctions.bind(_get, this)();
...
function waituntil(key, other) {
  ...
  return function (obj, prop, descriptor) {
    ...
    if (!toxicPredicateFunctions.isFunction(_value)) throw new TypeError('@waituntil can only be used on function, but not ' + _value);
    var binded = false;
    var waitingQueue = [];
    ...
    return {
      value: function value() {
        ...
        var boundFn = toxicPredicateFunctions.bind(_value, this);
        var runnable = toxicPredicateFunctions.bind(canIRun, this).apply(undefined, args);
        if (toxicPredicateFunctions.isPromise(runnable)) {
          return _Promise.resolve(runnable).then(function () {
            return toxicPredicateFunctions.bind(_value, _this).apply(undefined, args);
          });
        } else if (runnable === true) {
          return toxicPredicateFunctions.bind(_value, this).apply(undefined, args);
        } else {
...
function lazyInit(obj, prop, descriptor) {
  ...
  return {
    get: function get() {
      var value = toxicPredicateFunctions.bind(initializer, this)();
...
var Hooks = function () {
  ...
  _createClass(Hooks, [{
    ...
    value: function post(value, context) {
      ...
      toxicPredicateFunctions.bind(handler, context)(newVal, oldVal);
...
function watch(keyOrFn) {
  var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      deep = _ref2.deep,
      other = _ref2.other,
      omit = _ref2.omit;

  if (!toxicPredicateFunctions.isString(keyOrFn) && !toxicPredicateFunctions.isFunction(keyOrFn)) throw new TypeError('You must pass a function or a string to find the hanlder function.');
  ...
  return function (obj, prop, descriptor) {
    var handler = toxicPredicateFunctions.isString(keyOrFn) ? function (newVal, oldVal) {
      ...
      return toxicPredicateFunctions.bind(fn, this)(newVal, oldVal);
    } : keyOrFn;
    var hooks = new Hooks({
      prop: prop,
      handler: handler,
