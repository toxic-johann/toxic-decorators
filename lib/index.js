...
/**
 * toxic-decorators v0.4.0-beta.10
 * (c) 2017-2019 toxic-johann
 * Released under GPL-3.0
 * Built ad Sun Jun 02 2019 00:06:33 GMT+0800 (China Standard Time)
 */
...
function isAccessorDescriptor(desc) {
  return desc && (lodashEs.isFunction(desc.get) || lodashEs.isFunction(desc.set)) && lodashEs.isBoolean(desc.configurable) && lodashEs.isBoolean(desc.enumerable) && desc.writable === undefined;
}
function isDataDescriptor(desc) {
  return desc && desc.hasOwnProperty('value') && lodashEs.isBoolean(desc.configurable) && lodashEs.isBoolean(desc.enumerable) && lodashEs.isBoolean(desc.writable);
}
function isInitializerDescriptor(desc) {
  return desc && lodashEs.isFunction(desc.initializer) && lodashEs.isBoolean(desc.configurable) && lodashEs.isBoolean(desc.enumerable) && lodashEs.isBoolean(desc.writable);
}
...
function compressOneArgFnArray(fns) {
  ...
  if (!lodashEs.isArray(fns) || fns.length < 1) {
    throw new TypeError(errmsg);
  }

  if (fns.length === 1) {
    if (!lodashEs.isFunction(fns[0])) {
      throw new TypeError(errmsg);
    }
...
function warn(message) {
  if (lodashEs.isFunction(console.warn)) {
...
function getOwnKeysFn() {
  ...
  return lodashEs.isFunction(getOwnPropertySymbols) ? function (obj) {
...
function getOwnPropertyDescriptorsFn() {
  return lodashEs.isFunction(Object.getOwnPropertyDescriptors) ? Object.getOwnPropertyDescriptors : function (obj) {
...
function accessor() {
  ...
  if (!(lodashEs.isArray(get) && get.length > 0) && !(lodashEs.isArray(set) && set.length > 0) && !lodashEs.isFunction(get) && !lodashEs.isFunction(set)) {
  ...
  var singleFnGet = lodashEs.isArray(get) ? compressOneArgFnArray(get, errmsg) : get;
  var singleFnSet = lodashEs.isArray(set) ? compressOneArgFnArray(set, errmsg) : set;
  return function (obj, prop, descriptor) {
    ...
    var hasGet = lodashEs.isFunction(singleFnGet);
    var hasSet = lodashEs.isFunction(singleFnSet);

    var handleGet = function handleGet(value) {
      return hasGet ? lodashEs.bind(singleFnGet, this)(value) : value;
    };

    var handleSet = function handleSet(value) {
      return hasSet ? lodashEs.bind(singleFnSet, this)(value) : value;
    };

    if (isAccessorDescriptor(descriptor)) {
      var originGet = descriptor.get,
          originSet = descriptor.set;
      var hasOriginGet = lodashEs.isFunction(originGet);
      var hasOriginSet = lodashEs.isFunction(originSet);
      ...
      var getter = hasOriginGet || hasGet ? function () {
        ...
        var boundGetter = lodashEs.bind(handleGet, this);

        var originBoundGetter = function originBoundGetter() {
          return hasOriginGet ? lodashEs.bind(originGet, _this)() : undefined;
        };
      ...
      var setter = hasOriginSet || hasSet ? function (val) {
        ...
        var boundSetter = lodashEs.bind(handleSet, this);

        var originBoundSetter = function originBoundSetter(value) {
          return hasOriginSet ? lodashEs.bind(originSet, _this2)(value) : value;
        };
      ...
      return {
        get: function get() {
          var boundFn = lodashEs.bind(handleGet, this);

          if (inited) {
            return boundFn(_value);
          }

          _value = lodashEs.bind(initializer, this)();
          inited = true;
          return boundFn(_value);
        },
        set: function set(val) {
          var boundFn = lodashEs.bind(handleSet, this);
    ...
    return {
      get: function get() {
        return lodashEs.bind(handleGet, this)(value);
      },
      set: function set(val) {
        var boundFn = lodashEs.bind(handleSet, this);
  ...
  return function (obj, prop, descriptor) {
    ...
    var handler = function handler() {
      ...
      var paras = fns.reduce(function (paras, fn) {
        var result = lodashEs.bind(fn, _this).apply(void 0, _toConsumableArray(paras));
        return result === undefined ? paras : lodashEs.isArray(result) ? result : [result];
      }, args);
      return lodashEs.bind(fn, this).apply(void 0, _toConsumableArray(paras));
    };
...
function after() {
  ...
  return function (obj, prop, descriptor) {
    ...
    var handler = function handler() {
      var ret = lodashEs.bind(value, this).apply(void 0, arguments);
      return lodashEs.bind(fn, this)(ret);
    };
...
function initialize() {
  ...
  return function (obj, prop, descriptor) {
    ...
    if (isAccessorDescriptor(descriptor)) {
      ...
      return accessor({
        get: function get(value) {
          ...
          return lodashEs.bind(fn, this)(value);
        },
    ...
    if (isInitializerDescriptor(descriptor)) {
      ...
      var handler = function handler() {
        return lodashEs.bind(fn, this)(lodashEs.bind(initializer, this)());
      };
    ...
    var value = lodashEs.bind(fn, this)(descriptor.value);
    return {
      configurable: descriptor.configurable,
      enumerable: descriptor.enumerable,
      value: value,
...
function alias(other, key, option) {
  ...
  if (other !== undefined && !lodashEs.isObject(other)) {
    throw new TypeError(illegalObjErrorMsg);
  }

  var _ref3 = lodashEs.isPlainObject(option) ? option : {
    force: false,
    omit: false
  },
      force = _ref3.force,
  ...
  return function (obj, prop, descriptor) {
    ...
    function getTargetAndName(other, obj, key) {
      var target = !lodashEs.isObject(other) ? obj : other;
      var keys = key.split('.');
  ...
  return function () {
    ...
    return function (Klass) {
      ...
      getOwnKeys(prototype).concat(include).forEach(function (key) {
        ...
        if (key === 'constructor' && !construct || self && isClass && ['name', 'length', 'prototype'].indexOf(key) > -1 || exclude.indexOf(key) > -1 || lodashEs.isFunction(requirement) && requirement(prototype, key, desc, {
          self: self
        }) === false) {
          return;
        }
...
function getBoundSuper(obj, fn) {
  ...
  if (superStore.has(fn) === false) {
    superStore.set(fn, lodashEs.bind(fn, obj));
  }
  ...
  if (arguments.length === 1) {
    return classify(autobind, {
      requirement: function requirement(obj, prop, desc) {
        return isDataDescriptor(desc) && lodashEs.isFunction(desc.value);
      }
    })()(obj);
  }
...
function frozen(obj, prop, descriptor) {
  ...
  if (isAccessorDescriptor(descriptor)) {
    ...
    return {
      ...
      enumerable: false,
      get: function get() {
        var value = lodashEs.bind(_get, this)();
...
function waituntil(key) {
  ...
  if (!lodashEs.isFunction(key) && !toxicPredicateFunctions.isPromise(key) && !lodashEs.isString(key)) {
  ...
  return function (obj, prop, descriptor) {
    ...
    var canIRun = toxicPredicateFunctions.isPromise(key) ? function () {
      return key;
    } : lodashEs.isFunction(key) ? key : function () {
      var keys = key.split('.');
      ...
      var originTarget = !lodashEs.isObject(other) ? this : other;

      if (!binded) {
        var target = toxicUtils.getDeepProperty(originTarget, keys.slice(0, -1));

        if (lodashEs.isNil(target)) {
          return target;
        }
    ...
    return {
      ...
      value: function value() {
        ...
        var boundFn = lodashEs.bind(_value, this);
        var runnable = lodashEs.bind(canIRun, this).apply(void 0, args);

        if (toxicPredicateFunctions.isPromise(runnable)) {
          return Promise.resolve(runnable).then(function () {
            return lodashEs.bind(_value, _this).apply(void 0, args);
          });
        } else if (runnable === true) {
          return lodashEs.bind(_value, this).apply(void 0, args);
        }
...
function lazyInit(obj, prop, descriptor) {
  ...
  if (!lodashEs.isFunction(descriptor.initializer)) {
    if (process.env.NODE_ENV !== 'production') {
  ...
  return {
    ...
    enumerable: enumerable,
    get: function get() {
      var value = lodashEs.bind(initializer, this)();
...
function applyDecorators(Class, props) {
  ...
  var isPropsFunction = lodashEs.isFunction(props);

  if (isPropsFunction || lodashEs.isArray(props)) {
    if (!lodashEs.isFunction(Class)) {
  ...
  if (!self && !lodashEs.isFunction(Class)) {
  ...
  for (var key in props) {
    var value = props[key];
    var decorators = lodashEs.isArray(value) ? value : [value];
    var handler = void 0;
...
function deepProxy(value, hook, _ref) {
  ...
  var proxyValue = new Proxy(value, {
    get: function get(target, property, receiver) {
      ...
      if (lodashEs.isArray(target) && arrayChangeMethod.indexOf(property) > -1) {
        return function () {
          arrayChanging = true;
          lodashEs.bind(value, receiver).apply(void 0, arguments);
          arrayChanging = false;
          hook();
        };
      }
      ...
      if (lodashEs.isPlainObject(value) || lodashEs.isArray(value)) {
    ...
    set: function set(target, property, value) {
      ...
      var newVal = lodashEs.isPlainObject(value) || lodashEs.isArray(value) ? deepProxy(value, hook, {
        diff: diff,
        operationPrefix: operationPrefix
      }) : value;
...
function deepObserve(value, hook, _ref2) {
    ...
    return keys.reduce(function (props, key) {
      props[key] = [accessor({
        ...
        get: function get(val) {
          ...
          if (lodashEs.isPlainObject(val) || lodashEs.isArray(val)) {
  ...
  if (lodashEs.isArray(value)) {
    var methodProps = arrayChangeMethod.reduce(function (props, key) {
      props[key] = [initialize(function (method) {
        method = lodashEs.isFunction(method) ? method : Array.prototype[key];
        return function () {
          var originLength = value.length;
          arrayChanging = true;
          lodashEs.bind(method, value).apply(void 0, arguments);
          arrayChanging = false;
  ...
  var operateProps = (_operateProps2 = {}, _defineProperty(_operateProps2, operationPrefix + 'set', [initialize(function (method) {
    return function (property, val) {
      ...
      if (lodashEs.isFunction(method)) {
        lodashEs.bind(method, _this)(property, val, {
          disable: true,
          isNewVal: isNewVal
        });
      }
    ...
    return function (property) {
      if (lodashEs.isFunction(method)) {
        lodashEs.bind(method, _this)(property);
      } else {
...
function watch() {
  ...
  var option = lodashEs.isPlainObject(args[args.length - 1]) ? args[args.length - 1] : {};
  ...
  return function (obj, prop, descriptor) {
    var fns = args.reduce(function (fns, keyOrFn, index) {
      ...
      fns.push(lodashEs.isString(keyOrFn) ? function (newVal, oldVal) {
        ...
        return lodashEs.bind(fn, this)(newVal, oldVal);
      } : keyOrFn);
      return fns;
    }, []);
    ...
    var handler = function handler(newVal, oldVal) {
      ...
      fns.forEach(function (fn) {
        return lodashEs.bind(fn, _this2)(newVal, oldVal);
      });
    };
    ...
    return compressMultipleDecorators(accessor({
      set: function set(value) {
        ...
        var hook = function hook() {
          return lodashEs.bind(handler, _this3)(newVal, oldVal);
        };

        return deep && (lodashEs.isPlainObject(value) || lodashEs.isArray(value)) ? proxy ? deepProxy(value, hook, {
          diff: diff,
          operationPrefix: operationPrefix
      ...
      get: function get(value) {
        ...
        if (!inited) {
          ...
          var hook = function hook() {
            return lodashEs.bind(handler, _this4)(newVal, oldVal);
          };

          if (deep && (lodashEs.isPlainObject(value) || lodashEs.isArray(value))) {
            if (proxy) {
      ...
      set: function set(value) {
        ...
        if (!diff || oldVal !== value) {
          lodashEs.bind(handler, this)(newVal, oldVal);
        }
...
function runnable(key) {
  ...
  if (!lodashEs.isFunction(key) && !lodashEs.isString(key)) {
  ...
  return function (obj, prop, descriptor) {
    ...
    var canIRun = lodashEs.isFunction(key) ? key : function () {
      var keys = key.split('.');
      var originTarget = !lodashEs.isObject(other) ? this : other;
      return toxicUtils.getDeepProperty(originTarget, keys);
    };
    backup = lodashEs.isFunction(backup) ? backup : function () {};
    return {
      configurable: configurable,
      enumerable: false,
      writable: false,
      value: function value() {
        if (lodashEs.bind(canIRun, this).apply(void 0, arguments) === true) {
          return lodashEs.bind(_value, this).apply(void 0, arguments);
        }

        return lodashEs.bind(backup, this).apply(void 0, arguments);
      }
    };
  };
}
...
function string(defaultValue) {
  ...
  if (lodashEs.isFunction(defaultValue)) {
    args.unshift(defaultValue);
    defaultValue = '';
  } else if (!lodashEs.isString(defaultValue)) {
    defaultValue = '';
  }

  args.unshift(function (value) {
    return lodashEs.isString(value) ? value : defaultValue;
  });
...
function boolean(defaultValue) {
  ...
  if (lodashEs.isFunction(defaultValue)) {
    args.unshift(defaultValue);
    defaultValue = false;
  } else if (!lodashEs.isBoolean(defaultValue)) {
    defaultValue = false;
  }

  args.unshift(function (value) {
    return lodashEs.isBoolean(value) ? value : defaultValue;
  });
  return initialize.apply(void 0, args);
}
...
function array(defaultValue) {
  ...
  if (lodashEs.isFunction(defaultValue)) {
    args.unshift(defaultValue);
    defaultValue = [];
  } else if (!lodashEs.isArray(defaultValue)) {
    defaultValue = [];
  }

  args.unshift(function (value) {
    return lodashEs.isArray(value) ? value : defaultValue;
  });
  return initialize.apply(void 0, args);
}
...
function number(defaultValue) {
  ...
  if (lodashEs.isFunction(defaultValue)) {
    args.unshift(defaultValue);
    defaultValue = 0;
  } else if (!lodashEs.isNumber(defaultValue)) {
    defaultValue = 0;
  }

  args.unshift(function (value) {
    return lodashEs.isNumber(value) ? value : defaultValue;
  });
  return initialize.apply(void 0, args);
}
