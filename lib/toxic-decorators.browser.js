...
function bind(fn, context) {
  if (fn.bind) {
    ...
    return function __autobind__() {
      ...
      return fn.call.apply(fn, [context].concat(toConsumableArray(args)));
    };
  }
}
/**
  ...
  return function (obj, prop, _ref) {
    ...
    var handler = function handler() {
      ...
      var paras = fns.reduce(function (paras, fn) {
        var result = bind(fn, _this).apply(undefined, toConsumableArray(paras));
        return result === undefined ? paras : isArray(result) ? result
        // $FlowFixMe: what the hell, it can be anything
        : [result];
      }, args);
      return bind(fn, this).apply(undefined, toConsumableArray(paras));
    };
    return {
      value: handler,
      configurable: configurable,
  ...
  return {
    ...
    get: function get$$1() {
      ...
      var boundFn = function boundFn() {
        ...
        return fn.call.apply(fn, [_this].concat(toConsumableArray(args)));
      };
...
function waituntil(fn, other) {
  ...
  return function (obj, prop, _ref) {
    ...
    if (isString(fn)) {
      ...
      defineProperty$3(target, fn, desc);
    }
    return {
